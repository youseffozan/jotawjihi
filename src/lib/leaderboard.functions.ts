...
export const getLeaderboard = createServerFn({ method: "GET" }).handler(async () => {
  try {
    ...
    const { data: profiles, error: profilesError } = await supabaseAdmin
      .from("profiles")
      .select("id, full_name, first_name, last_name, grade, field, is_public")
      .in("id", userIds);
    ...
    const entries: LeaderboardEntry[] = userIds.map((uid) => {
      ...
      const displayName = isPublic
        ? ([firstName, lastName].filter(Boolean).join(" ") || "طالب")
        : (firstName || "طالب مجهول");
      return {
        userId: uid,
        name: displayName,
        firstName: isPublic ? firstName : firstName,
        lastName: isPublic ? lastName : null,
        grade: (p?.grade as string | null) ?? null,
        field: isPublic ? ((p?.field as string | null) ?? null) : null,
    ...
    console.warn("Using dummy leaderboard because Supabase is not configured:", error);
